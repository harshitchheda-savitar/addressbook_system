...
public class QueryConstants {
	...
	public static final String UPDATE_CONTACT_DETAILS = "UPDATE contact C INNER JOIN address_book A ON A.id=C.addressbook_name_id SET C.first_name = ? AND C.last_name = ? AND C.aadhar = ? AND C.address = ? AND C.city = ? AND C.state = ? AND C.zip = ? WHERE A.is_active = 1 AND C.is_active = 1 AND A.addressbook_name = ? AND A.addressbook_type_id = ? AND C.first_name = ? AND C.last_name = ? AND C.aadhar = ?";
